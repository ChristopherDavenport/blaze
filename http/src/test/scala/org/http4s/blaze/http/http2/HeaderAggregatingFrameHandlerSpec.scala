...
class HeaderAggregatingFrameHandlerSpec extends Specification {
  ...
  "HEADERS frame with compressors" should {
    ...
    "Make a round trip with a continuation frame" in {
      ...
      val bs2 = Http20FrameSerializer.mkContinuationFrame(1, true, hsBuf)
    ...
    "Make a round trip with a continuation frame" in {
      ...
      val bs2 = Http20FrameSerializer.mkContinuationFrame(1, true, encodeHeaders(hs))
      decoder.decodeBuffer(BufferTools.joinBuffers(bs2)) must_== Halt
    }
    ...
    "Make a round trip with a continuation frame" in {
      ...
      val bs2 = Http20FrameSerializer.mkContinuationFrame(1, true, encodeHeaders(hs2))
      decoder.decodeBuffer(BufferTools.joinBuffers(bs2)) must_== Halt
    }
    ...
    "Fail on invalid frame sequence (bad streamId)" in {
      ...
      val bs2 = Http20FrameSerializer.mkContinuationFrame(2, true, BufferTools.emptyBuffer)
    ...
    "Fail on invalid frame sequence (wrong frame type)" in {
      ...
      val bs2 = Http20FrameSerializer.mkWindowUpdateFrame(2, 1)
  ...
  "PUSH_PROMISE frame" should {
    ...
    "make round trip" in {
      val buff1 = BufferTools.joinBuffers(Http20FrameSerializer.mkPushPromiseFrame(1, 2, true, 0, dat))
      dec(1, 2, true).decodeBuffer(buff1) must_== Continue
      buff1.remaining() must_== 0
    }

    "preserve padding" in {
      val buff = addBonus(Http20FrameSerializer.mkPushPromiseFrame(1, 2, true, bonusSize, dat))
      dec(1, 2, true).decodeBuffer(buff) must_== Continue
      buff.remaining() must_== bonusSize
    }

    "fail on bad stream ID" in {
      Http20FrameSerializer.mkPushPromiseFrame(0 , 2, true, -10, dat) must throwA[Exception]
    }

    "fail on bad promised stream ID" in {
      Http20FrameSerializer.mkPushPromiseFrame(1 , 0, true, -10, dat) must throwA[Exception]
      Http20FrameSerializer.mkPushPromiseFrame(1 , 3, true, -10, dat) must throwA[Exception]
    }

    "fail on bad padding" in {
      Http20FrameSerializer.mkPushPromiseFrame(1 , 2, true, -10, dat) must throwA[Exception]
      Http20FrameSerializer.mkPushPromiseFrame(1 , 2, true, 500, dat) must throwA[Exception]
    }
  }
  ...
  "PUSH_PROMISE frame with header decoder" should {
    ...
    "Make a simple round trip" in {
      val hs = Seq("foo" -> "bar", "biz" -> "baz")
      val hsBuf = encodeHeaders(hs)
      val bs = Http20FrameSerializer.mkPushPromiseFrame(1, 2, true, 0, hsBuf)
    ...
    "Make a round trip with a continuation frame" in {
      val hs = Seq("foo" -> "bar", "biz" -> "baz")
      val hsBuf = encodeHeaders(hs)
      val bs = Http20FrameSerializer.mkPushPromiseFrame(1, 2, false, 0, hsBuf)
      ...
      val bs2 = Http20FrameSerializer.mkContinuationFrame(1, true, BufferTools.emptyBuffer)
      decoder.decodeBuffer(BufferTools.joinBuffers(bs2)) must_== Halt
    }
    ...
    "Make a round trip with a continuation frame" in {
      ...
      val bs = Http20FrameSerializer.mkPushPromiseFrame(1, 2, false, 0, BufferTools.emptyBuffer)
    ...
    "Make a round trip with a continuation frame" in {
      val hs1 = Seq("foo" -> "bar")
      val hs2 = Seq("biz" -> "baz")
      val bs = Http20FrameSerializer.mkPushPromiseFrame(1, 2, false, 0, encodeHeaders(hs1))
    ...
    "Fail on invalid frame sequence (wrong streamId)" in {
      ...
      val bs2 = Http20FrameSerializer.mkContinuationFrame(2, true, encodeHeaders(hs2))
      decoder.decodeBuffer(BufferTools.joinBuffers(bs2)) must beAnInstanceOf[Error]
    }
