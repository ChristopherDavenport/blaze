...
final class HeaderDecoder(maxHeaderListSize: Int, val maxTableSize: Int) { self =>
  ...
  final def setMaxHeaderTableSize(max: Int): Unit = decoder.setMaxHeaderTableSize(max)
  ...
  final def discard(buffer: ByteBuffer, streamId: Int, endHeaders: Boolean): MaybeError =
    doDecode(buffer, streamId, endHeaders, HeaderDecoder.nullListener)

  private def doDecode(buffer: ByteBuffer, streamId: Int, endHeaders: Boolean, listener: HeaderListener): MaybeError = {
    try {
      ...
      COMPRESSION_ERROR.goaway(s"Compression error on stream $streamId").toError
    }
  }
}
