...
class Http2ClientSelector(config: HttpClientConfig) {
  ...
  private[this] def newH2Settings(): Http2Settings = {
    val settings = Http2Settings.default()
    settings.maxHeaderListSize = config.maxHeaderLength + config.maxRequestLine // the request line is part of the headers
    settings
  }
