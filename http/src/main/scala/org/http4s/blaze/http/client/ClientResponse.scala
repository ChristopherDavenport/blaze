...
import org.http4s.blaze.http.{MessageBody, _}
...
/** HTTP response received by the client
  ...
  */
case class ClientResponse(code: Int, status: String, headers: Headers, body: MessageBody) {
  ...
  private def getCharset(hs: Headers): String = {
    headers.collectFirst {
      case (k, v) if k.equalsIgnoreCase("content-type") => ClientResponse.charsetRegex.findFirstIn(v)
