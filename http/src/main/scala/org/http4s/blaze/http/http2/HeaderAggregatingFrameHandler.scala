...
/** A [[Http20FrameHandler]] that decodes raw HEADERS, PUSH_PROMISE,
...
private[http2] abstract class HeaderAggregatingFrameHandler(
    ...
    headerDecoder: HeaderDecoder) extends Http20FrameHandler {
  ...
  final override def onHeadersFrame(streamId: Int,
    ...
    if (inHeaderSequence()) {
      return PROTOCOL_ERROR.goaway(s"Received HEADERS frame while in in headers sequence. Stream $streamId").toError
    }

    if (buffer.remaining() > inboundSettings.maxHeaderListSize) {
      ...
      return headerSizeError(buffer.remaining(), streamId)
    }
  ...
  final override def onPushPromiseFrame(streamId: Int,
    ...
    if (inHeaderSequence()) {
      ...
      return PROTOCOL_ERROR.goaway(msg).toError
    }
  ...
  final override def onContinuationFrame(streamId: Int, endHeaders: Boolean, buffer: ByteBuffer): Http2Result = {
    ...
    if (hInfo.streamId != streamId) {
      val msg = s"Invalid CONTINUATION frame: stream Id's don't match. Expected ${hInfo.streamId}, received $streamId"
      return PROTOCOL_ERROR.goaway(msg).toError
    }
    ...
    if (totalSize > inboundSettings.maxHeaderListSize) {
      // TODO: this can legally be handled with a 431 response, but the headers must be processed to keep
      // TODO: the header decompressor in a valid state. https://tools.ietf.org/html/rfc7540#section-10.5.1
      return headerSizeError(totalSize, streamId)
    }
