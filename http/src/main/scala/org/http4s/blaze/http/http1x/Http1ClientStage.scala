...
import org.http4s.blaze.http.client.{ClientResponse, HttpClientConfig, HttpClientSession}
...
private class Http1ClientStage(config: HttpClientConfig)
  extends TailStage[ByteBuffer] with HttpClientSession {
  ...
  def prepareForDispatch(): Boolean = stageLock.synchronized {
    if (getStatus == HttpClientSession.Ready) {
      codec.reset()
      dispatchId += 1
      true
    }
  ...
  override def quality: Int = 100  // TODO: is there anything interesting we could do with this for http/1.x?

  override def close(): Unit = stageLock.synchronized {
    state match {
      case Closed(_) => // nop
      case _ =>
        stageShutdown()
  ...
  def apply(request: HttpRequest): Future[ClientResponse] = stageLock.synchronized {
    state match {
  ...
  private def receiveResponse(buffer: ByteBuffer, p: Promise[ClientResponse]): Unit = stageLock.synchronized {
    ...
    if (!buffer.hasRemaining) channelReadThenReceive(p)
      ...
      val response = ClientResponse(prelude.code, prelude.status, prelude.headers, body)
      p.success(response)
    }
    else {
  ...
  private def channelReadThenReceive(p: Promise[ClientResponse]): Unit = {
    state match {
  ...
  private class ClientMessageBody(private[this] var buffer: ByteBuffer) extends MessageBody { body =>
    ...
    override def discard(): Unit = body.synchronized {
    ...
    private def readAndParseBody(p: Promise[ByteBuffer]): Unit = {
      channelRead().onComplete { result =>
        body.synchronized { result match {
          case Success(b) =>
            buffer = b
            parseBody(p)
  ...
  private def getBody(buffer: ByteBuffer): MessageBody = {
    if (codec.contentComplete()) MessageBody.emptyMessageBody
    else {
      logger.debug("Content is not complete. Getting body reader.")
      new ClientMessageBody(buffer)
    }
  }
